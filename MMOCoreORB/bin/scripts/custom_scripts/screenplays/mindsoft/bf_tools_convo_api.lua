-- BY: Mindsoft											-- last edited: 5.13.2018 by Nexxus	-- (this code belongs soley to WarfrontEMU)
...
------------------------------------------------(DYNAMIC NEXT CONVO HANDLER)							-- (this code belongs soley to WarfrontEMU)
...
------------------------------------------------(GENERIC SCREEN HANDLER)							-- (this code belongs soley to WarfrontEMU)
	...
	if (memberC:isGrouped()) then
			...
			if (groupMember ~= nil) and memberC:isInRangeWithObject(groupMember,100) and (SceneObject(groupMember):isPlayerCreature()) then --validate member
				...
				if (factRest == nil) or (factRest =="imperial" and creature:isImperial()) or (factRest == "rebel" and creature:isRebel()) or (factRest == "neutral" and creature:isNeutral()) then
					...
					if (state+1) == qEnd then --last kill state
						...
						suiManager:sendMessageBox(groupMember, groupMember, "Warfront Quest Update:", wpE[7], "@ok", "bf_tools", "questUpdate")
		...
		if (factRest == nil) or (factRest =="imperial" and memberC:isImperial()) or (factRest == "rebel" and memberC:isRebel()) or (factRest == "neutral" and memberC:isNeutral()) then
			...
			if (state+1) == qEnd then
				...
				local suiManager = LuaSuiManager()
				suiManager:sendMessageBox(player, player, "Warfront Quest Update:", wpE[7], "@ok", "bf_tools", "questUpdate")
				--addWaypointSpecial(pPlayer, clear, wpPlanet[1], wpName[2], wpDesc, wpX[3], wpY[4], wpColor[5], wpActive, wpNotify, wpGroupID[6])
...
function bf_tools:item_Purchase( pPlayer, reward, purchaseCost, test )
	...
	for i = containerSize - 1 , 0 , -1 do
        	...
        	invObj = LuaSceneObject(pInvObj)
     	    	-- Get Objectnames from Each Item and remove if Warfront coin until cost = 0.
...
function bf_tools:reward( pPlayer, reward_type, reward, modifier, test )
	...
	elseif reward_type == "update" then
		...
		-- SUI Quest Update popup.
			local suiManager = LuaSuiManager()
			suiManager:sendMessageBox(pPlayer, pPlayer, "Warfront Quest Update:", test, "@ok", "bf_tools", "questUpdate")
		return true
...
local pBedroom = getSceneObject(4005944) -- Ranch House bedroom
	...
	elseif reward_type == "screenplay" then
		...
		-- SUI Quest Update popup.
			...
			if test[3] then
				suiManager:sendMessageBox(pPlayer, pPlayer, "Warfront Quest Update:", test[3], "@ok", "bf_tools", "questUpdate")
				if smart then
				...
				suiManager:sendMessageBox(pPlayer, pPlayer, "Warfront Quest Update:", suiMessage, "@ok", "bf_tools", "questUpdate")
			end
			return true
		...
		elseif  (not creature:hasScreenPlayState( reward, modifier ) and test == nil or creature:hasScreenPlayState( test, modifier )) then
			creature:setScreenPlayState( reward, modifier ) 
			creature:sendSystemMessage("-MISSION UPDATED-")
		-- SUI Quest Update popup.
			local suiManager = LuaSuiManager()
...
----------------------------------------------(Warfront Tools Special Functions)----------------------------------------------	-- (this code belongs soley to WarfrontEMU)
...
--( Property of WarfrontEMU.net )-- (this code belongs soley to WarfrontEMU)
