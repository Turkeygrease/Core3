...
function gcw_transit_system:start() 
   ...
   local pImpPilotEndor = spawnMobile("endor", "imp_transitpilot", 60, -3093, 200, 5277, -90, 0 )

   local pImpPilotNaboo = spawnMobile("naboo", "imp_transitpilot", 60, -4863, 6, 4116, 1, 0 )
   local pImpPilotTatooine = spawnMobile("tatooine", "imp_transitpilot", 60, -8.6, 1, 7.0, 89, 1280132 )
...
--   local pImpPilotCoro = spawnMobile("Corellia", "imp_transitpilot", 60, 135, 28.0, -4705, 35, 0 )
			...
			if (creature:getFaction() == 3679112276) then
				...
				if (optionLink == "sPvP" and playerObj:getFactionStanding("imperial") >= 0) then
	        		        playerObj:decreaseFactionStanding("imperial",0)
            				player:switchZone("endor", -3103.0, -200.0, 5277.3, 0)
				elseif (optionLink == "sPvP" and playerObj:getFactionStanding("imperial") < 0) then
					nextConversationScreen = conversation:getScreen("notenough_faction")
					creature:sendSystemMessage("You dont have enough FP")
				end				
--[[				-- xxxxxxxxxx PvP Battleground xxxxxxxxxxxx	 	
				if (optionLink == "sPvPPlanet" and playerObj:getFactionStanding("imperial") >= 0) then
	        		        playerObj:decreaseFactionStanding("imperial",0)
            				player:switchZone("rori", 5520.5, 80.2, 5623.8, 0)
					createEvent(1, "pvp", "handlePvpZone", conversingPlayer,"")
				elseif (optionLink == "sPvPPlanet" and playerObj:getFactionStanding("imperial") < 0) then
					nextConversationScreen = conversation:getScreen("notenough_faction")
					creature:sendSystemMessage("You dont have enough FP")
				end
				-- xxxxxxxxxx PvP Battleground xxxxxxxxxxxx	 	
				if (optionLink == "sPvPJakku" and playerObj:getFactionStanding("imperial") >= 0) then
	        		        playerObj:decreaseFactionStanding("imperial",0)
            				player:switchZone("jakku", 1.5, 16, 22.2, 610000111)
					createEvent(1, "pvp", "handlePvpZone", conversingPlayer,"")
				elseif (optionLink == "sPvPJakku" and playerObj:getFactionStanding("imperial") < 0) then
					nextConversationScreen = conversation:getScreen("notenough_faction")
					creature:sendSystemMessage("You dont have enough FP")
				end	
]]			
