...
#ifndef FORCESHROUDCOMMAND_H_
#define FORCESHROUDCOMMAND_H_
...
class ForceShroudCommand : public CombatQueueCommand {
public:

	ForceShroudCommand(const String& name, ZoneProcessServer* server)
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (creature->getHAM(CreatureAttribute::ACTION) < actionCost) {
			creature->sendSystemMessage("@jedi_spam:force_shroud_too_tired"); // You do not have enough action to active Force Shroud.
			return false;
		}
		...
		if (creature->hasBuff(buffCRC2)) {
			creature->sendSystemMessage("@jedi_spam:force_shroud_fail"); // You use Force Shroud at this time!
		} else if (!creature->hasBuff(buffCRC2)) {
			Locker locker(buff);
			Locker locker2(buff2);
			creature->sendSystemMessage("@jedi_spam:force_shroud_activate"); // You manifest a Force Shroud to protect yourself.
			...
			creature->playEffect("clienteffect/entertainer_smoke_bomb_level_3.cef", "");
			...
			creature->sendSystemMessage("@jedi_spam:force_shroud_protected"); // You are already protected by a Force Shroud.
