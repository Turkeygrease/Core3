...
#ifndef INSPIRATIONCOMMAND_H_
#define INSPIRATIONCOMMAND_H_
...
class InspirationCommand : public CombatQueueCommand {
public:

	InspirationCommand(const String& name, ZoneProcessServer* server)
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (creature->getHAM(CreatureAttribute::ACTION) < actionCost) {
			creature->sendSystemMessage("@cbt_spam:bh_shield_too_tired"); // You do not have enough action to activate your shields.
			return false;
		}
		...
		if (creature->hasBuff(buffCRC2)) {
			creature->sendSystemMessage("@cbt_spam:bh_shield_fail"); // You cannot activate a Shield at this time!
		} else if (!creature->hasBuff(buffCRC2)) {
			Locker locker(buff);
			Locker locker2(buff2);
			creature->sendSystemMessage("@cbt_spam:bh_shield_activate"); // You activate a shield to protect yourself.
			...
			creature->sendSystemMessage("@cbt_spam:bh_shield_protected"); // You are already protected by a Shield.
