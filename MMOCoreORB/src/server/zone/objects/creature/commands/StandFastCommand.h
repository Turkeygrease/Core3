...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (creature->getHAM(CreatureAttribute::ACTION) < actionCost) {
			creature->sendSystemMessage("@cbt_spam:commando_standfast_too_tired"); // You do not have enough action to Stand Fast.
			return false;
		}
		...
		if (creature->hasBuff(buffCRC)) {
			creature->sendSystemMessage("@cbt_spam:commando_standfast_protected"); // You are alreadying Standing Fast.
		} else if (creature->hasBuff(buffCRC2)) {
			creature->sendSystemMessage("@cbt_spam:commando_standfast_fail"); // You cannot Stand Fast at this time!
		} else {
			Locker locker(buff);
			Locker locker2(buff2);
			creature->sendSystemMessage("@cbt_spam:commando_standfast_activate"); // You Stand Fast to protect yourself.
